...
在 `packages/server/src/ai/scenarios.ts`，先确保从 context 导入 `OUTPUT_GUARD`：
...
在 `packages/web-admin/test/reviewTab.test.tsx` 的 `describe("ReviewTab", ...)` 内追加：
...
```tsx
  it("saves current unblurred notes before transcribing", async () => {
    ...
    expect(calls).toEqual(["put", "transcribe"]);
...
```bash
git add packages/server/src/activities/service.ts packages/server/test/attendanceSummary.test.ts packages/web-admin/src/pages/tabs/SummaryTab.tsx packages/web-admin/test/activities.test.tsx packages/web-admin/test/notifyStatus.test.tsx
